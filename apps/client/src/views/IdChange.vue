<template>
  <ion-page>
    ...
    <ion-content class="ion-padding-top">
      <ion-radio-group v-model="documentType" @ion-change="documentID = ''">
        <ion-list-header>
      ...
      <form @submit.prevent="saveId()" @keyup.enter="saveId()">
        ...
        >
          <ion-input
            required
            v-maska:[maskInputOptions]
            v-model="documentID"
            type="text"
            autocapitalize="characters"
            _AutoCapitalizeNotWorking
            :maxlength="documentType === 'passport-other' ? 30 : 9"
...
Preferences.get({
  key: KEY_ID,
}).then(({ value }) => (documentID.value = value || ''));

const maskInputOptions = reactive({
  mask: () =>
    documentType.value === 'dni'
      ? '########$'
      : documentType.value === 'nie'
      ? '$#######$'
